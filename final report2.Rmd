...
"Predicted close" and decrease the processing time of our classifier, we
...
print(co_result) sensitivity(as.table(co_result))
specificity(as.table(co_result)) precision(as.table(co_result))

acc \<- co_result\$overall["Accuracy"] acc
...
acquired2Clust1<-print(sum(dataset$opean[kmeans.result$cluster =="2"]== "acquired", na.rm = TRUE))
